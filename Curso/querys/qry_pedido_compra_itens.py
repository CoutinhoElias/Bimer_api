...
class PedidosDeCompraItens:
    ...
    def _obter_token(self):
        ...
        #     "nonce": "123456789",
        #     "password": "f2c0d35e8965bf9d5201769880749ff5", # CÓDIGO DO USUARIO FIREBASE
        # }
    ...
    def ler_arquivo_pedido_de_compra(self, diretorio):
        ...
        df_itens_planilha['id_classificacao_fiscal'] = None
        ...
        nova_ordem = ['id_fornecedor', 'codigo_fornecedor', 'fornecedor', 'codigo_produto', 'id_produto','id_classificacao_fiscal', 'id_unidade', 'nome_produto', 
    ...
    def criar_pedido_de_compra_itens_api(self, dicionario_final):
        print(json.dumps(dicionario_final, indent=4))
        """Consulta a API para obter os itens de um pedido de compra pelo seu ID"""
        # URL do endpoint da API para consultar o pedido de compra
        url = "http://192.168.254.1:8091/api/compras/pedidos"
        ...
        # Converte o dicionário para JSON e faz a solicitação POST para a API do pedido de compra
        response = requests.post(url, json=dicionario_final, headers=headers)

        # Verifica se a solicitação foi bem-sucedida (200 ou 201)
        if response.status_code not in [200, 201]:
            # self.open_dialogo("Existe um erro, consulte o Elias (85-98643-9530)!")
            # Lança uma exceção se a solicitação falhar
            raise Exception(f"Erro na solicitação do PedidoDeVenda API: {response.status_code}, {response.text}")
